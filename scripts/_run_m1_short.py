...
out_dir = Path('results/m1_solver_ablation_v2')
out_dir.mkdir(parents=True, exist_ok=True)
structure_labels = assign_structure_label(ds.ground_truth, ds.source_flows, ds.target_flows)

solvers = ['thresholded_cost', 'greedy_nn', 'hungarian', 'balanced_ot', 'rc_uot']
all_metrics = []
all_stratified = []
...
for sname in solvers:
    ...
    print(f'  T shape={result.T.shape}, mass={result.T.sum():.4f}, rt={rt:.1f}s')
    ...
    ci = m.precision_ci
    print(f'  P={m.precision:.4f} [{ci.lower_95:.4f},{ci.upper_95:.4f}] R={m.recall:.4f} F1={m.f1:.4f}')
...
csv_path = out_dir / 'solver_ablation.csv'
...
strat_csv = out_dir / 'by_structure.csv'
