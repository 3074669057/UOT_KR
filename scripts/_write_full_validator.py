...
Independent validation entry point for provider-submitted cross-chain flow data.
Does NOT import experiment runner, solver, or confirmation modules.

Usage:
    python scripts/validate_rc_uot_provider_sample.py --dataset path/to/dataset_root
    python scripts/validate_rc_uot_provider_sample.py --dataset path/to/dataset_root --contract path/to/handoff
"""
import argparse
import csv
import hashlib
...
def validate_directory_structure(dataset_root, report):
    ...
    if not root.is_dir():
        ...
        report.add_check('directory_exists', False, f'Not found: {dataset_root}')
        return False
    ...
    for fname in ['source_flows.jsonl', 'target_flows.jsonl', 'collection_manifest.json']:
        ...
        if not fpath.is_file():
            report.reject('P0_REJECTED_DIRECTORY', EXIT_REJECTED_DIRECTORY)
            report.add_check(f'required_file_{fname}', False, f'Missing: {fpath}')
            return False
        report.add_check(f'required_file_{fname}', True)
    for forbidden_name in FORBIDDEN_NAMES:
        fpath = root / forbidden_name
        if fpath.is_dir():
            report.reject('P0_REJECTED_LABEL_LEAKAGE', EXIT_REJECTED_LABEL_LEAKAGE)
            report.add_check('no_label_leakage', False, f'Forbidden directory: {fpath}')
            return False
        if '.' in forbidden_name and fpath.is_file():
            report.reject('P0_REJECTED_LABEL_LEAKAGE', EXIT_REJECTED_LABEL_LEAKAGE)
            report.add_check('no_label_leakage', False, f'Forbidden file: {fpath}')
            return False
...
def validate_schema(dataset_root, contract_dir, report):
    ...
    for name in ['source_flow', 'target_flow', 'collection_manifest']:
        schema_path = os.path.join(contract_dir, f'{name}.schema.json')
        if not os.path.isfile(schema_path):
            report.add_check(f'schema_load_{name}', False, f'Schema not found: {schema_path}')
            report.reject('P0_REJECTED_SCHEMA', EXIT_REJECTED_SCHEMA)
            return False
        schemas[name] = _load_json(schema_path)
        report.add_check(f'schema_load_{name}', True)
    ...
    with open(sp, 'r', encoding='utf-8') as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                report.reject('P0_REJECTED_SCHEMA', EXIT_REJECTED_SCHEMA)
                report.add_check('schema_source', False, f'Line {line_no}: JSON error')
                return False
            try:
            ...
            if sid in source_ids:
                report.semantic_errors.append({'file': 'source_flows.jsonl', 'line': line_no,
                    'message': f'Duplicate source_flow_id: {sid}'})
            source_ids.add(sid)
    report.source_count = len(source_ids)
    report.add_check('schema_source', len(report.schema_errors) == 0, f'{report.source_count} records')
    ...
    with open(tp, 'r', encoding='utf-8') as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                report.reject('P0_REJECTED_SCHEMA', EXIT_REJECTED_SCHEMA)
                report.add_check('schema_target', False, f'Line {line_no}: JSON error')
                return False
            try:
            ...
            if tid in target_ids:
                report.semantic_errors.append({'file': 'target_flows.jsonl', 'line': line_no,
                    'message': f'Duplicate target_flow_id: {tid}'})
            target_ids.add(tid)
    report.target_count = len(target_ids)
    report.add_check('schema_target', len(report.schema_errors) == 0, f'{report.target_count} records')
    ...
    if os.path.isfile(policy_path):
        ...
        if expected_body and actual_body != expected_body:
            report.semantic_errors.append({'file': 'candidate_generation_policy.json',
                'message': f'policy_body_sha256 mismatch: expected {expected_body}, computed {actual_body}'})
            report.reject('P0_REJECTED_SEMANTICS', EXIT_REJECTED_SEMANTICS)
...
def validate_anti_triviality(dataset_root, contract_dir, report):
    ...
    if report.source_count < min_src:
        report.semantic_errors.append({'file': 'source_flows.jsonl',
            'message': f'source_flow_count ({report.source_count}) < min ({min_src})'})
    ...
    if report.target_count < min_tgt:
        report.semantic_errors.append({'file': 'target_flows.jsonl',
            'message': f'target_flow_count ({report.target_count}) < min ({min_tgt})'})
        report.reject('P0_REJECTED_TRIVIAL', EXIT_REJECTED_TRIVIAL)
    report.add_check('anti_triviality_counts',
        report.source_count >= min_src and report.target_count >= min_tgt,
        f'sources={report.source_count}, targets={report.target_count}')
...
def write_reports(report, output_dir):
    ...
    lines = [f'# Validation Report', '',
             f'- **Decision**: {report.decision}', f'- **Exit Code**: {report.exit_code}',
             f'- **Sources**: {report.source_count}', f'- **Targets**: {report.target_count}',
             f'- **Schema Errors**: {len(report.schema_errors)}',
             f'- **Semantic Errors**: {len(report.semantic_errors)}', '', '## Checks']
...
def main():
    ...
    if report.exit_code == EXIT_ACCEPTED:
        ...
        print(f'REJECTED: {report.decision}')
    sys.exit(report.exit_code)
...
with open('scripts/validate_rc_uot_provider_sample.py', 'w', encoding='utf-8') as f:
    f.write(body)
print('Validator written successfully')
