"""PHASE 6 (deep): run CURRENT pipeline on the FROZEN synthetic inputs directly."""
...
for seed in (42, 43, 44, 45, 46):
    ...
    out_dir = AUDIT / "celer_regression_delta" / "frozen_inputs_current_code" / f"seed_{seed}"
    ...
    rows.append({"seed": seed, "split": ev.get("split_recovery_rate"), "merge": ev.get("merge_recovery_rate"),
                 "edges": ev.get("num_predicted_edges")})
    print(rows[-1], flush=True)
pd.DataFrame(rows).to_csv(AUDIT / "celer_regression_delta" / "frozen_inputs_current_code.csv", index=False)
