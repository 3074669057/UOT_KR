...
def validate_provider_directory(dataset_dir):
    ...
    if not dir_ok:
        ...
        if "P0_REJECTED_LABEL_LEAKAGE" in err_codes:
            status = "P0_REJECTED_LABEL_LEAKAGE"
        else:
            status = "P0_REJECTED_DIRECTORY"
        return ProviderValidationReport(status=status,schema_valid=False,semantic_valid=False,overlap_valid=False,candidate_gate_valid=False,issues=issues,statistics=stats,input_hashes=hashes)
    try:
